describe('basic formatting', () => {
    ...
    it('can write text with multiple selected font-size', () => {
        ...
        cy.get(`a[data-command="fontSize"][data-option="12"]`).click();
        cy
            .get(editorSelector)
            .should('include.html', `<div><span style="font-size:12px;">hello world 12</span></div>`);

        cy.get(editorSelector).type('{selectall}');
        buttonSelector.click(); // open dropdown

        cy.get(`a[data-command="fontSize"][data-option="24"]`).click();
        cy
            .get(editorSelector)
            .should('include.html', `<div><span><span style="font-size:24px;">hello world 12</span></span></div>`)
            .type('{enter}');

        cy.get(editorSelector).type('{selectall}');
        buttonSelector.click(); // open dropdown

        cy.get(`a[data-command="fontSize"][data-option="18"]`).click();
        cy
            .get(editorSelector)
            .should('include.html', `<div><span><span><span style="font-size:18px;">hello world 12</span></span></span></div>`)
            .type('{enter}');
    });
